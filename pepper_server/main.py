...
def process_audio(audio_queue, whisper_model, llama_handler: Llama):
    """
...
def serve():
    ...
    processing_thread = Thread(
        target=process_audio, args=(audio_queue, whisper_model, llama_handler), daemon=True
    )
    processing_thread.start()

    # Start the gRPC server
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    pb2_grpc.add_MediaServiceServicer_to_server(MediaManager(audio_queue), server)
